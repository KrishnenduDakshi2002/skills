...
2. Read existing tests and public contract documentation. [Derive behavioral invariants](references/coverage-dimensions.md#derive-behavioral-invariants) from the legacy implementation/API, relevant frontend consumers, and newer implementation. Reconcile their evidence and approved contract changes before choosing expectations; surface missing sources and disagreements.
...
- Keep all new test bodies unimplemented. Do not add assertions, hooks, dependency mocks, production imports, model instances, fixture builders, DB setup, or refactors. Permit only suite/case declarations, inert parameter tables describing conditions/expected outcomes, and concise source-evidence or contract-question comments.
...
Review the outline against the traced code, layer strategy, and coverage dimensions before handing it off. Report selected endpoints, spec paths and pending case counts by layer (expand parameter rows), derived invariants and their evidence sources, existing coverage reused, reasons any layer needs no separate suite, unresolved questions/missing sources, and stage-2 infrastructure needs. Keep the case inventory in the specs. Do not report pending cases as implemented, passing, or measured coverage. Avoid running integration setup just to enumerate declarations.
...
- For every reviewed invariant, identify the implemented cases and observable assertions that protect it; report any remaining gap. Check that a plausible violation (wrong tenant, extra response field, duplicate side effect, missing write) would fail the test. Avoid count targets, snapshot dumps, and assertions that merely echo mocks.
