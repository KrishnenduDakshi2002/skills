...
description: Create or extend behavior-focused unit and integration coverage for selected TagMango external API endpoints in two stages, first reviewable case-only .spec.ts files, then implementation in those same files after review. Use for external API test outlines, coverage reviews, implementing approved suites, and shared Mongo document builder migration needed by those tests. Excludes end-to-end tests and live endpoint certification.
---

# Cover External API

Protect external API business rules, public responses, persisted outcomes, and side effects through unit and integration tests. Make the case inventory readable before implementing it. Keep the same `.spec.ts` files through both stages; never substitute a Markdown test plan.
...
4. Apply the [layer testing strategy](references/test-layers.md): assess DTO/validation, service, repository/schema, response adapters, and their integration separately. Give each rule an owning suite and add focused cross-layer cases for wiring and combined outcomes. Use real repositories and memory Mongo where query correctness matters. Keep external providers fake.
...
1. Re-read the reviewed specs, current source, and review feedback. Reconcile drift before implementation. Preserve case intent and file paths; turn `todo` into `it` and remove pending modifiers as bodies are completed. Do not delete, merge away, weaken, or leave approved cases skipped just to produce a green run. Make newly discovered gaps visible as additional declarations; seek a decision only where contract or scope materially changes.
2. Follow [testing infrastructure](references/testing-infrastructure.md) before executing any integration suite, including old consumer tests. Reuse/consolidate the shared builders and make the harness safe before importing code that could connect to infrastructure.
...
4. Implement independent, deterministic cases at their assigned layers. Use real rules and collaborators material to the behavior; mock direct dependencies for units and fake external providers for integration. A service test with a mocked repository does not establish repository rules or persistence correctness. Never stub the behavior under test or reconstruct the expected value by calling its production mapper/calculator.
...
7. Assert persistence with targeted reads through the suite connection. Assert queue/provider/cache calls only when they are the externally observable contract; verify payload, multiplicity, and absence on rejected/no-op paths when applicable. Avoid private-method spies, repository call order, exact query syntax, or incidental internal call counts.
...
- Run focused unit and integration targets separately after verifying their setup is safe. Check Jest discovery includes the intended library specs and does not accidentally classify integration suites as units. Do not disable safety guards to work around a runner failure.
...
- Report implemented/passing/failing/pending counts, commands actually run, remaining behavioral/boundary gaps, and infrastructure/refactor changes. Separate outline completeness from runtime results; report blocked execution honestly. Do not claim all external APIs are covered from one batch or claim E2E/live certification.
